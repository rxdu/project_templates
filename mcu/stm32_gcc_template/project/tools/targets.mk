...
VTMPL_COMMAND=$(PYTHON2) tools/python/versionTemplate.py $< $@
#$(BIN)/$(lastword $(subst /, ,$@))
...
DFU_COMMAND=$(PYTHON2) tools/python/dfu-convert.py -b $(LOAD_ADDRESS):$< $@
...
CLEAN_COMMAND=rm -f $(PROG_PREFIX)*.elf $(PROG_PREFIX)*.hex $(PROG_PREFIX)*.bin $(PROG_PREFIX)*.dfu $(PROG_PREFIX)*.map $(BIN)/dep/*.d $(BIN)/*.o
