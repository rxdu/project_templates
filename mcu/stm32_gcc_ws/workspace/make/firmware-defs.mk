...
ifeq ($(FW_BUILD_CONF), debug)
export DEBUG:=YES
CCACHE := $(CCACHE_BIN)
else ifeq ($(FW_BUILD_CONF), default)
# In the default case, keep the old "DEBUG"  variable handling
CCACHE := $(CCACHE_BIN)
else ifeq ($(FW_BUILD_CONF), release)
export DEBUG:=NO
else
$(error Only debug, release, or default allowed for FW_BUILD_CONF)
endif

# Define toolchain component names.
CC      = $(CCACHE) $(TCHAIN_PREFIX)gcc
CXX     = $(CCACHE) $(TCHAIN_PREFIX)g++
LD      = $(CCACHE) $(TCHAIN_PREFIX)gcc
AR      = $(TCHAIN_PREFIX)ar
AS      = $(TCHAIN_PREFIX)as
